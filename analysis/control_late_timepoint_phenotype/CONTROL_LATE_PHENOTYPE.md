...
2. **Late timepoint** — the imaging rig runs on two interleaved ~3 h cadences,
   ...
   "pass"), and each strain gets its **latest pass within [80, 110] h**; all
   colonies from that pass are used as the strain's replicates.
...
- `timepoint_h` = rounded hour of the strain's latest pass (105 or 108).
...
- **314/320** strain rows; 286 strains with ≥3 colonies at their latest pass,
  28 flagged (`n_colonies < 3`); 1 strain has no control (Cu = 0) image in
  the 80–110 h window and 5 have no late control image at all.
- Timepoint passes used: 105.0 h (84 strains) and 108.0 h (230 strains).
