#!/bin/bash
#SBATCH --job-name=tierB_gwasc
#SBATCH --partition=short
#SBATCH --cpus-per-task=4
#SBATCH --mem=40G
#SBATCH --time=2:00:00
#SBATCH --output=slurm_tierB_gwasc_%j.out
#SBATCH --error=slurm_tierB_gwasc_%j.err
#SBATCH --nodes=1
set -euo pipefail
...
"$PY" scripts/tierb_set_tests.py \
  ...
  --prefix gwasc --mc 50000 \
  > tierb_run_gwasc.log 2>&1
echo "TIERB_DONE rc=$?" >> tierb_run_gwasc.log
