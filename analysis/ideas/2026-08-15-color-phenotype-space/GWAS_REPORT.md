...
`{gwas,gwasc}_<trait>_assoc.csv.gz` (gzipped, ~27 MB each raw), multi-trait
`multi_{gwas,gwasc}_{color,copper}_fisher_top500.csv`.
...
`bslmm_<trait>.hyp.txt.gz` (per-sample PVE/PGE/pge/pi/n_gamma trace), `bslmm_<trait>.gamma.txt.gz`
(per-MCMC-sample 300-cluster inclusion matrix; value = representative variant index, 0 = inactive;
both gzipped — the raw gamma files are 59–123 MB each).
...
- Co-localization: `results/gwas/tierB/coloc/coloc_final.csv.gz` (gzipped), `coloc_anchors.csv`,
  `coloc_enrichment.txt`, `results/gwas/figures/coloc_dxy_fst.{png,pdf}`

**Figure regeneration** (reproduce exactly; cwd = analysis folder; PY = the repo pixi python):

```bash
PY=.pixi/envs/default/bin/python   # or the pixi env python3
$PY scripts/make_loco_figure.py \
  --merged results/gwas/loco/loco_merged_gwas.csv results/gwas/loco/loco_merged_gwasc.csv \
  --tiera-summary results/gwas/tierA_summary/tiera_summary.csv \
  --out-dir results/gwas/figures

$PY scripts/make_tierb_figure.py \
  --tierb results/gwas/tierB/tierb_settests_gwas.csv results/gwas/tierB/tierb_settests_gwasc.csv \
  --mcver results/gwas/tierB/tierb_skat_mcver_gwas.csv results/gwas/tierB/tierb_skat_mcver_gwasc.csv \
  --out-dir results/gwas/figures

$PY scripts/make_coloc_figure.py \
  --coloc-final results/gwas/tierB/coloc/coloc_final.csv.gz \
  --anchors results/gwas/tierB/coloc/coloc_anchors.csv \
  --dxy results/gwas/pixy/genome_dxy.txt \
  --fst results/gwas/pixy/genome_fst.txt \
  --out-dir results/gwas/figures
```

2026-08-16/17 dataviz revision (following methodology review): LOCO panel A now overlays the
full-kinship Tier-A λ per trait and shows the full per-scaffold LOCO λ range (not just the
median); LOCO panel B uses the **same-SNP** full-kinship p (via `top_rs_TierA`/`top_p_TierA`),
fixing a comparison that previously plotted the trait-level max SNP (which for gwasc-chroma is
a different locus than the scaffold_10 anchor). Tier B panel B is linear in `-log10 p` (was
log-scaled) with the exact-MC top-50 overlaid as crosses. Co-localization panel A draws the
80th-pct high-dxy/high-Fst thresholds; panel B adds Wilson 95% CIs and in-figure
Fisher-exact OR/p (re-computed p matches `coloc_enrichment.txt`: dxy OR=0.81 p=0.612,
Fst OR=0.41 p=0.065).

## 9. Tier D/E/G — locus→gene mapping, fine-mapping, and prior-locus replication

Follow-ups from the divergence review (`09-NEXT-GWAS-DESIGN.md` §7): map Tier-A/BSLMM loci to
genes (D), fine-map anchors into credible sets (E), and test whether the prior lab's growth-rate
locus (chr13:13_30149, p=1.68e-11 in `SNP_GWAS_T37C_linear.tsv`) replicates in our scans (G).

### 9.1 Tier D — locus → gene annotation

- Annotator: `scripts/annotate_gwas_loci.py`; gene index = 6,799 mRNA-derived genes
  (`results/gwas/tierD/scripts/gene_index.json`, product/GO/InterPro/PFAM).
- **FDR-sig SNPs annotated:** `tierD_fdr_snps_annotated.csv.gz` (12,348 rows, all FDR-sig;
  per-trait SNP counts restate the FDR set: AUC_0 7340 / AUC_10 3989 / resilience_30 524 /
  chroma 345 / AUC_30 31 / AUC_20 56 / sat 40 / cu_dose_slope 22 / IC50_est 1).
- **Independent loci** (positional 250 kb clump, one lead per retained region — *caveat*: the
  clump keeps only the single most-significant SNP per chromosome as `lead`, so a chromosome
  with two distant (<250 kb) signals keeps only the top SNP's neighborhood; e.g. chr13's 217
  FDR-sig AUC_10 SNPs form one 525 kb-spanning block that collapses to a single lead
  `scaffold_13_791853` p=2.4e-7): `tierD_independent_loci.csv.gz` (5,286 rows:
  AUC_0 2832 / AUC_10 1636 / resilience_30 404 / chroma 282 / AUC_20 56 / sat 40 /
  cu_dose_slope 22 / AUC_30 13 / IC50_est 1).
- **Notable mapped anchors** (FDR-sig, product in `tierD_independent_loci.csv.gz`):
  - chroma: scaffold_10:384905 → OM429_004636 (hypothetical); scaffold_3:406583 →
    OM429_001484 (hypothetical); scaffold_8:831789 → **OM429_004009 telomerase
    reverse transcriptase** (chromosome-stability candidate).
  - AUC_10: scaffold_10:396172 → OM429_004640 **RNA-dependent ATPase = DBP3**;
    scaffold_2:1002173 → OM429_001126 RNA-pol-I TF; scaffold_7:1043473 →
    OM429_003643 endodeoxyribonuclease.
- **BSLMM top-PIP loci** (`tierD_bslmm_loci_annotated.csv`, 8 loci, 7 overlapping a gene):
  chroma scaffold_3:26959 → OM429_001347 (hypo, PF11911-adjacent region), 137161 →
  OM429_001381 (hypo), 132903 → **OM429_001379 methionine aminopeptidase 1**,
  7440 → OM429_001340 (hypo); AUC_10 scaffold_5:533603 → OM429_002543 (hypo),
  scaffold_2:939277 → OM429_001102 (hypo); resilience_30 scaffold_16:563722 → 232 bp from
  OM429_006384, scaffold_2:71223 → OM429_000783.
- **Replication-gene answer (G-bridge): the prior chr13 growth-rate locus maps inside
  OM429_005439** (scaffold_13:30,096–30,670; hypothetical protein, no GO/InterPro/PFAM
  annotation) — see §9.3.

### 9.2 Tier E — fine-mapping (Wakefield ABF credible sets)

- Script: `scripts/finemap_credible_sets.py`; 14 anchors × 90/95/99% sets →
  `results/gwas/tierE/tierE_credible_sets.csv` (42 rows). Method: z-space ABF (scale-free),
  prior on the non-centrality parameter SD = 0.2, logsumexp-normalized PIPs, candidates
  restricted to `p_wald < 1e-3` (without the filter the sets span thousands of null SNPs).
  Rare variants (af ~0.015) flagged `rare_driven=True`.
- Representative results (95% CS):
  - **chroma_lead_10** (scaffold_10 anchor): CS n=24, lead pp=0.054, beta=1.107±0.190 (SE),
    CI [0.73, 1.48], af=0.806 — a common-variant, well-bounded signal.
  - chroma BSLMM scaffolds 3a/b/c: CS n=98/131/131, lead pp ≈ 0.01 each — un-resolvable
    (the BSLMM chroma clusters are diffuse).
  - **auc10_lead_10** (scaffold_10:396172 DBP3): CS n=67, lead af=0.015, beta=8.24e5±1.39e5,
    **rare_driven=True** — the DBP3 hit is a rare-variant signal, wide set.
  - **resil_lead_13** (scaffold_13:810026): 95% CS n=1, lead pp=0.615, af=0.015 (99% CS n=1,
    lead pp=0.977) — a fully-resolved rare singleton.
  - **ic50_lead_16**: CS n=10, lead pp=0.183.
  - cu_slope anchor scaffold_3:546065: lead pp=0.351.
- Implication: the Tier-A anchors split into common, well-mapped signals (chroma_10) and
  rare-variant signals whose sets stay wide (auc10 DBP3, resilience scaffold_13) — LD
  (n=201 strains) limits resolution at rare AF, consistent with BSLMM's near-oligogenic call.

### 9.3 Tier G — replication of the prior growth-rate locus chr13:13_30149

- Prior lab (UCR population-genomics, `SNP_GWAS_T37C_linear.tsv`, n=731,202 rows): chr13:30149
  is the top hit for T37C linear growth rate, snpID 100002, **p = 1.676e-11**; other top hits
  12:600910 (2.06e-15), 1:2147308 (1.1e-13). That scan is a growth-rate trait; our panel is
  color/copper-response, so "replication" is tested as: does any of our traits show signal at
  the locus?
- Our exact position 13_30149 was not genotyped (MAF-filtered/absent); the closest proxy
  **scaffold_13_30134 (15 bp away)** is **FDR-significant for AUC_10** — p_wald=4.03e-6
  (score-test p=7.90e-6), beta=804,778, af=0.015. All other traits are nominally null there
  (best bunch: clone_mean_area p=0.044, bright p=0.082, both non-FDR).
- Context: AUC_10's 217 chr13 FDR-sig SNPs form one block spanning 11,701–800,664 that
  collapses to a single lead (scaffold_13_791853, p=2.4e-7). The prior locus (and our proxy)
  sits inside that same chr13 rare-haplotype block, so the growth-rate locus **does replicate**
  in our growth phenotype as part of the chr13 signal — but as a rare-EF effect (af≈0.015–0.03),
  indistinguishable from the chr13 block's other SNPs without denser genotyping.
- **Gene at the locus:** OM429_005439 (scaffold_13:30,096–30,670), a hypothetical protein with
  no annotation; flanking genes also hypothetical (OM429_005440) / Ark1-family Ser/Thr kinase
  (OM429_005441, 37,357–41,776). The causal candidate under the replicated locus remains
  functionally unannotated.

### 9.4 Tier D/E/G outputs

- `results/gwas/tierD/`: `tierD_fdr_snps_annotated.csv.gz` (12,348 rows),
  `tierD_independent_loci.csv.gz`,
  `tierD_bslmm_loci_annotated.csv`, `scripts/gene_index.json`
- `results/gwas/tierE/tierE_credible_sets.csv` (42 rows)
- **Figure — Tier D gene map + Tier E credible-set resolution**
  (`results/gwas/figures/tierde_gene_finemap.{png,pdf}`, script
  `scripts/make_tierde_figure.py`):
    - Panel A: the scaffold_10 anchor region (chroma lead 384,905 & AUC_10/DBP3 lead
      396,172, ~11 kb apart): -log10 p of the FDR-significant SNPs per trait with a
      schematic gene track of the genes the window spans.
    - Panel B: for the 14 Tier-E anchors, the 95% credible-set size (log x-axis) vs lead
      posterior probability, coloured by the rare-driven flag (af<0.05).
- Reproduce: `PY .pixi/envs/default/bin/python; $PY scripts/annotate_gwas_loci.py --fdr
  results/gwas/fdr --index results/gwas/tierD/scripts/gene_index.json --outdir
  results/gwas/tierD; $PY scripts/finemap_credible_sets.py; $PY scripts/make_tierde_figure.py
  --sig results/gwas/tierD/tierD_fdr_snps_annotated.csv.gz --credset
  results/gwas/tierE/tierE_credible_sets.csv --out-dir results/gwas/figures

## 10. Methods (draft publication paragraph — Tier A→G chain)

**Population.** 201 haploid *Rhodotorula* sp. isolate genomes (7 genus-representative spp.,
near-clonal within the focal clade) were QC'd (biallelic SNPs, MAC≥5, missingness<10% → 404,706
variants), genetic relatedness estimated as the centered GRM from 20,769 LD-pruned variants
(PLINK2 `--indep-pairwise 50 5 0.2`), and phenotypes measured as clone-means over ~4 replicate
plates (color: ColorLab chroma / HSV saturation, brightness; growth: colony area; copper
response: AUC0–30 μg/mL, AUC-ratio tolerance, resilience_30 = AUC(30)/AUC(0), cu-dose slope,
IC50).

**Tier A — single-SNP GWAS.** Genome scans used a kinship-only multivariate linear mixed model
(GEMMA `-lmm 4 -k`, Wald/likelihood test; chromosome-integer codes per the source project's
chrom map). Covariate (PC/ancestry) models were infeasible (singular GRM from the near-clone
strains), so kinship-only was kept as the valid model. Scans were run on all-201 and
near-clone-culled-173 sets; because GEMMA 0.98.3 lacks phenotype-permutation, significance was
set by Benjamini–Hochberg FDR (q<0.05) on the full genomic p-values.

**Tier B — set tests.** Two-Window (burden, SKAT-variance-component via three-moment Liu
approximation, and window-min single-SNP) tests were computed for 178 pixy high-dxy 100-kb
windows × 12 traits × 2 sets; SKAT approximated p were Monte-Carlo-verified on the top-50
windows (r=0.984 in log10 space); no set-level signal survived FDR.

**Tier C — architecture.** Bayesian sparse LMM (GEMMA `-bslmm 1`, 100k MCMC, 20% burn-in) on 5
representative traits partitioned PVE/PGE and active LD-cluster count n_gamma.

**LOCO sensitivity.** For the three interpretable anchors (chroma scaffold_10, AUC_10
scaffold_10, resilience_30 scaffold_13), full genomic scans were re-run
leave-one-chromosome-out (kinship recomputed per excluded chromosome); every anchor
reproduced at unchanged p, confirming the full-kinship signals were not chromosome-proximal
artifacts.

**Tier D — gene mapping.** FDR-significant SNPs and BSLMM top-PIP loci were mapped to the
6,799-gene mRNA annotation (overlap, else nearest gene within 250 kb) and collapsed by
positional clumping (250 kb) into independent lead loci; products/GO/InterPro/PFAM were pulled
for each hit gene.

**Tier E — fine-mapping.** Fourteen anchors (Tier-A leads + BSLMM peaks + replication locus)
were fine-mapped with Wakefield approximate-Bayes-factor credible sets computed in z-space
(scale-free), NCP prior SD=0.2, PPIs normalized via logsumexp on candidates with p_wald<1e-3;
sets were reported at 90/95/99%, and rare-driven (af<0.05) anchors flagged.

**Tier G — replication of a prior growth-rate locus.** Our scans were probed at the previously
reported growth-rate top hit (chr13:13_30149; nearest proxy 13 bp away, scaffold_13_30134):
the proxy is FDR-significant in our AUC_10 scan (p_wald=4.0e-6) inside a rare-haplotype chr13
block, and the locus maps inside hypoth. gene OM429_005439.
