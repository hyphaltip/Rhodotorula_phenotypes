...
|------|------|-----------------|---------|
| 01 | Statistical Physicist | Arrest is Cu-invariant in shape (Weibull k≈0.45 flat); pooled dispersion sd(log₁₀area) 0.30→0.43 **is largely a size/floor artifact** — within-strain size-controlled slope≈0 (p=0.46) except `R. taiwanensis` widens (−0.014, p=0.03) and `R. paludigena` narrows (p=0.001) | `idea01_{arrest_collapse,dispersion}.csv`, `fig01_{master_collapse,arrest_exponent,gibrat_dispersion}.png`; `idea01b_{within_strain,species_table,size_confound}.csv`, `fig01b_{within_strain_slopes,size_vs_dispersion}.png` |
...
| 10 | Within-species | **Within-species (strain) variation is the DOMINANT scale of variation for every trait (62–92% of variance): colony size least species-structured (38% between) vs chroma/Cu-slope ~8–13% between.** Boxplots/panel per trait (n≥3/species) show wide within-species overlap | `idea10_variance_decomposition.csv`, `idea10_species_variation.csv`, `fig10_boxplot_{5 traits}.png` |
| 11 | Power & variants | **GWAS power: R. mucilaginosa = 178 effective independent haplotypes (200 tips, redundancy 1.12); min detectable per-SNP R² ≈ 0.16 genome-wide / 0.10 candidate at n≈201 — only large-effect loci (≈0.7 SD/allele, ≈22–25% genetic var) findable; Cu-slope unmappable (ICC 0.19). Existing SNP VCF (NRRL-Y2510 project, 728k SNPs × 422 strains, haploid) already covers 201 of our 278 phenotyped strains → no variant calling needed, just GWAS on our color traits** | `idea11_power.csv`, `idea11_effective_n.csv`, `idea11_detectable_effect.csv`, `idea11_genome_redundancy.csv`, `fig11_power_curves.png` |
